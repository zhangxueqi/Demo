package cn.zhang.opensource.util.excel.impl;
...
import cn.zhang.opensource.util.date.DateUtil;
import cn.zhang.opensource.util.excel.ExcelHelper;
import cn.zhang.opensource.util.reflect.ReflectUtil;
import cn.zhang.opensource.util.string.StringUtil;
import jxl.Cell;
import jxl.Sheet;
import jxl.Workbook;
...
public class JxlExcelHelper extends ExcelHelper {
	...
	public <T> void writeExcel(Class<T> clazz, List<T> dataModels,
		...
		//对excel的写入操作过程： 创建工作簿->创建工作表->创建sheet->表头的添加(设计格式)->内容的添加(设计内容类型的问题)		
